// File: app/(dashboard)/dashboardAsset/components/NotificationWidget.tsx
// Versi perbaikan dengan tipe data 'branch'

"use client";
import React from 'react';
import { Branch } from '@prisma/client';

// DIUBAH: Tipe data menggunakan 'branch'
type AssetWithDetails = {
    id: string;
    productName: string;
    branch: { // Diganti dari location
        name: string;
    };
                ...
                {notifications.map(asset => (
                    <div key={asset.id} className={`flex items-start gap-3 p-3 rounded-md ${asset.notification?.type === 'error' ? 'bg-red-50' : 'bg-yellow-50'}`}>
                        ...
                        <div>
                            ...
                            {/* --- DIUBAH DI SINI --- */}
                            <p className="text-xs font-medium text-gray-500">{asset.branch.name}</p>
