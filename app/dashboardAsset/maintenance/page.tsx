// File: app/dashboardAsset/maintenance/page.tsx
...
export default function MaintenancePage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [activitiesRes, assetsRes] = await Promise.all([
        fetch('/api/assets/activities'), fetch('/api/assets')
      ]);
  ...
  const handleDeleteMaintenanceClick = async (id: string) => { if (confirm('Yakin ingin menghapus jadwal ini?')) { try { const res = await fetch(`/api/assets/maintenance/${id}`, { method: 'DELETE' }); if (!res.ok) throw new Error('Gagal'); fetchData(); } catch (e) { alert('Gagal menghapus'); } } };
  const handleReturnClick = async (asset: Asset) => { if (confirm(`Yakin ingin mengembalikan aset "${asset.productName}"?`)) { try { const res = await fetch('/api/assets/return', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ assetId: asset.id }) }); if (!res.ok) { const err = await res.json(); throw new Error(err.message); } fetchData(); } catch (e: any) { alert(`Error: ${e.message}`); } } };

  // --- FUNGSI BARU UNTUK SELESAI & GAGAL ---
  const handleUpdateMaintenanceStatus = async (maint: Maintenance, newStatus: 'COMPLETED' | 'CANCELLED') => {
    const actionText = newStatus === 'COMPLETED' ? 'menyelesaikan' : 'menggagalkan';
    // Mengambil nama aset dari objek maintenance yang dilewatkan
    const assetName = (maint as any).asset?.productName || 'aset ini'; 

    if (confirm(`Yakin ingin ${actionText} maintenance untuk "${assetName}"?`)) {
        try {
                                        ...
                                        <div className="flex items-center gap-x-4">
                                            ...
                                            {act.type === 'MAINTENANCE' && act.originalMaintenance && (
                                                <>
                                                    ...
                                                    <button onClick={() => handleDeleteMaintenanceClick(act.originalMaintenance!.id)} className="text-red-600 hover:text-red-800 font-medium">🗑️ Hapus</button>
                                                </>
                                            )}
                                        </div>
                                    </td>
